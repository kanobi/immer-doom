...
#define ST_MSGHEIGHT  1  // Or shall I say, in lines?
#define ST_OUTTEXTX   0
#define ST_OUTTEXTY   6
#define ST_OUTWIDTH  52  // Width, in characters again. 
#define ST_OUTHEIGHT  1  // Height, in lines. 
...
pixel_t *st_backing_screen;  // graphics are drawn to a backing screen and blitted to the real screen 
...
ST_Responder (event_t* ev)
{
    ...
    else if (ev->type == ev_keydown) // if a user keypress...
    {
        ...
        {
            if (cht_CheckCheat(&cheat_god, ev->data2)) // 'dqd' cheat for toggleable god mode
            {
            ...
            else if (cht_CheckCheat(&cheat_ammonokey, ev->data2)) // 'fa' cheat for killer fucking arsenal
            {
            ...
            else if (cht_CheckCheat(&cheat_ammo, ev->data2)) // 'kfa' cheat for key full ammo
            {
            ...
            for (i=0;i<6;i++) // 'behold?' power-up cheats
            {
...
void ST_doPaletteStuff(void)
{
    ...
    {
        bzc = 12 - (plyr->powers[pw_strength]>>6); // slowly fade the berzerk out
        if (bzc > cnt)
            cnt = bzc;
    }
    ...
    // In Chex Quest, the player never sees red.  
    // Instead, the radiation suit palette is used to tint the screen green,
...
static void ST_loadUnloadGraphics(load_callback_t callback)
{
    ...
    for (int i=0; i<6; i++)
    {
        ...
        callback(namebuf, &arms[i][0]); // gray #
