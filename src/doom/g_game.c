...
gamestate_t     oldgamestate; // Gamestate the last time G_Ticker was called.
...
int             gameepisode; 
int             gamemap; 
int             timelimit; // If non-zero, exit the level after this number of minutes.
...
{ 
    ...
    { 
        ...
        {
            side += sidemove[speed]; // strage right
        }
        if (gamekeydown[key_left] || mousebuttons[mousebturnleft])
        {
            side -= sidemove[speed]; // strafe left
        }
        if (joyxmove > 0) 
            side += sidemove[speed]; 
        if (joyxmove < 0) 
