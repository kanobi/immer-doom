...
boolean D_Display (void)
{
    ...
    switch (gamestate)
    {
      case GS_LEVEL:
        ...
        if (wipe || 
            (viewheight != SCREENHEIGHT && fullscreen) || 
...
void D_RunFrame()
{
    ...
    {
        // here the actual rendering starts in D_Display()
        if ((wipe = D_Display()))
        {
...
void D_DoomMain (void)
{
    ...
    // address.
    //
    arg_parameter = M_CheckParmWithArgs("-query", 1);
    if (arg_parameter)
    {
        NET_QueryAddress(myargv[arg_parameter+1]);
        exit(0);
    }
    ...
    {
        int scale = 200;
        if (arg_parameter<myargc-1)
            scale = atoi (myargv[arg_parameter+1]);
        if (scale < 10)
            scale = 10;
        if (scale > 400)
            scale = 400;
    ...
    if (arg_parameter)
    {
        char *uc_filename = strdup(myargv[arg_parameter + 1]);
    ...
    //
    arg_parameter = M_CheckParmWithArgs("-skill", 1);
    if (arg_parameter)
    {
        startskill = myargv[arg_parameter+1][0]-'1';
        autostart = true;
    }
    ...
    //
    arg_parameter = M_CheckParmWithArgs("-episode", 1);
    if (arg_parameter)
    {
    ...
    //
    arg_parameter = M_CheckParmWithArgs("-timer", 1);
    if (arg_parameter)
    {
        timelimit = atoi(myargv[arg_parameter+1]);
    }
    ...
    {
        if (gamemode == commercial)
            startmap = atoi (myargv[arg_parameter+1]);
        else
        {
            startepisode = myargv[arg_parameter+1][0]-'0';
    ...
    arg_parameter = M_CheckParm("-testcontrols");
    if (arg_parameter > 0)
    {
        startepisode = 1;
        startmap = 1;
    ...
    //
    arg_parameter = M_CheckParmWithArgs("-record", 1);
    if (arg_parameter)
    {
